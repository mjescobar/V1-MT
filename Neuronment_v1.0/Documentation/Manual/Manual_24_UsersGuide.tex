...
The \gls{neuronmentsequencer} is the module inside Neuronment on charge of interpreting the \glssymbol{neuronmentprocedure} file specified at the program call. It bases its operation on loading settings and executing commands as indicated on the \glssymbol{neuronmentprocedure} file.

\subsection{Commands}

All the calculations and information retrieval of Neuronment are managed by \glssymbol{neuronmentprocedure} file lines named ``commands''.
...
The following section describes the different aspects of the \gls{neuronmentsequencersyntax} as it is required to be used on the development of a \glssymbol{neuronmentprocedure} file.
...
A \glssymbol{neuronmentprocedure} file should be written in ascii and it will be divided in lines using the line breaks as line termination. The resulting lines will be interpreted according their content.
