...
private:
  ...
  /**
   * Takes charge of the rescue directives
   * @param TokensP tokens from the file line
   * @return true if successful
   */
  static bool RescueCall(vector<string> TokensP);

  /**
   * Processes printing directive depending on the ConfigurationManager involved
   * @param TempManager The ConfigurationManager from where the info will be retrieved
   * @param TokensP The tokens from the file line parsing
   * @return true if successful
   */
  static bool ReportCallSettingPrint(SettingsManager &TempManager, vector<string> TokensP);
