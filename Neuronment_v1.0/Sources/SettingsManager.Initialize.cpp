...
void SettingsManager::InitializeVariables()
{
  ...
  for (int i = 0; i < MAX_V1_NEURONS; i++) {
    Variables.SetSetting(string("SIM:") + V1_LABEL + NNumber(i), Data_double, 8);
  }
  ...
  for (int i = 0; i < MAX_MT_NEURONS; i++) {
    Variables.SetSetting(string("SIM:") + MT_LABEL + NNumber(i), Data_double, 6);
  }
  ...
  for (int i = 0; i < MAX_V1_NEURONS; i++) {
    for (int j = 0; j < MAX_V1_NEURONS; j++) {
      Variables.SetSetting(string("SIM:") + V1_LABEL + NNumber(i) + ":" + V1_LABEL + NNumber(j), Data_double, 1);
    }
  }
  // MTMT Link Methods
  // None
