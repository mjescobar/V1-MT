...
ReturnType LogManager::InitializeMessages()
{
  ...
  ReturnCatch(MessagesLabel.PutEntryQuick("IN-002", "Trying to report an undeclared variable"));
  ReturnCatch(MessagesText.PutEntryQuick("IN-002", "The Simulator as well as the Simulation Environment has a list of predefined variables so store and retrieve information. The variable been addressed is not part of the list. The user is not allowed to create new variables."));
  // IN-003
  ...
  ReturnCatch(MessagesLabel.PutEntryQuick("IN-006", "Flag not found or without a value"));
  ReturnCatch(MessagesText.PutEntryQuick("IN-006", "The command on execution requires the definition of a flag (a word that starts with \"-\" in the arguments) that is not present or without a value (if a flag requires a value, the next word after it should be the string representing the value for the flag. This string must NOT start with a \"-\". If the value is a negative number put it between quotes)."));
  // IN-007
  ...
  ReturnCatch(MessagesLabel.PutEntryQuick("DV-002", "Trying to declare a previously declared setting"));
  ReturnCatch(MessagesText.PutEntryQuick("DV-002", "At the moment of creating a new variable for use by Neuronment, it has been detected that a previous variable with the same name has already been declared."));
  // DV-003
  ...
  ReturnCatch(MessagesImplementationAssert.PutEntryQuick("DV-013", true));
  ReturnCatch(MessagesRuntimeAssert.PutEntryQuick("DV-013", false));
  ReturnCatch(MessagesDisabling.PutEntryQuick("DV-013", 0));
  ReturnCatch(MessagesCount.PutEntryQuick("DV-013", 0));
  ReturnCatch(MessagesLabel.PutEntryQuick("DV-013", "Hash Table Full"));
  ReturnCatch(MessagesText.PutEntryQuick("DV-013", "The hash table has all the keys filled with values. The hash tables in Neuronment does not store lists under colissions."));
  // DV-014
